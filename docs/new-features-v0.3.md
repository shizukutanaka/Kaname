...
> **実装状況 (2026-09, D144)**: この設計のドメイン→共有インフラ解決は DNS
> 実装が存在せず注入経路 (`register_domain`) の呼出元もゼロだったため削除済み。
> 現行の PCR は `unknown:<送信ドメイン>` バケットと `pattern:auth_fail:subject_*`
> 構造クラスタのみで動作する。以下のコードスケッチは設計意図の記録であり
> 実装ではない。
