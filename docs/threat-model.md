...
**Residual risk**: 現時点ではメールパイプライン自体が未配線 (D10) のため `parse_auth_results` に実メールが流れておらず、悪用経路は存在しない。ただし**配線時に独立検証を入れないと、認証系シグナル全体が受信サーバへの盲目的信頼の上に乗る**。残作業は `docs/gap-analysis.md` D18 を参照。
