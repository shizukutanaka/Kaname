...
**このスコープの外にあるもの** (次段階):
添付ダウンロード、MLS 暗号化 (モック)、ローカル LLM 推論 (スタブ)。
...
| 添付ダウンロード | **未実装** (残る唯一の D10 項目)。`download_url` は保持されるだけ |

**受信→保存→表示→送信→検索の経路はすべて実装済み**である。
残るのは添付ダウンロードのみ (`docs/gap-analysis.md` D10)。
