...
| 本文リスク検出 (HTMLスマグリング/テキストQR/CSS外部参照) | ✅ 動作 | 同上 |
| 機微情報検出 (DLP, Inbound) | ✅ 動作 | 同上 |
| キャンペーン検出 (複数メール横断) | ✅ 動作 | 「フォルダを一括解析」 |

動作確認用のサンプルを [`examples/emails/`](../examples/) に同梱している。

**このスコープの外にあるもの** (次段階、いずれもネットワークが前提):
サーバとのメール送受信 (JMAP)、永続化 (SQLCipher)、アカウント設定 UI、検索、
...
到達可能クレートは 10 → **11** に増加。LLM 未配線でも BEC が動くよう
`BecDetector::deterministic_only()` (`NullLlm`) を追加し、**LLM という要件自体を削除**した
(10 シグナルファミリーのうち 9 つはモデル不要の決定論的ロジックであるため)。

**依然としてメールの取得元は `mock_emails()` である** (D10)。実メールが流れるように
なれば同じ経路がそのまま実データを処理する。
