...
現在 **16/22** が出荷バイナリに含まれる (2026-09 に不要判定済みの
billing/continuity/i18n/tray をワークスペースから削除し 27→23 へ。
kaname-core は唯一の利用箇所 (`EmailRow.triage`) が UI に表示されない
dead 出力だったため削除済み — D97)。
残る 6 個は**意図的に含めていない**:
...
| `kaname-ai` | Dual-LLM 北極星機構。LLM 配線未完了のため呼出元なしだが、製品の中核公約であり意図的に残置 (D1) |
| `kaname-error` | 共通エラー型。全クレートが固有エラー型で推移したため実利用ゼロ — 削除候補 (PR #231 で D85 として削除提案済み) |
