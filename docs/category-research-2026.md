...
**現状**: UI は日本語ハードコードのみ。フロント側の `src/i18n.ts` + `src/locales/` は
`t()` の実呼び出しがゼロの空転基盤だったため 2026-09 に削除 (E9)。
Rust 側の `kaname-i18n` クレートも重複のため同時期に削除 (D19)。
多言語化が必要になった時点で git 履歴から復元する。
