...
### Fixed
- fix(kaname-dlp,kaname-ui): 受信側 DLP が構造的に空だった問題を修正 (D104) — 既定ルール全5件が `Direction::Outbound` のため `Direction::Inbound` で評価する `scan_dlp_inbound` は常に所見ゼロを返し、UI の「機微情報の検出 (DLP)」表示が一度も出なかった。Inbound 既定ルール3件 (受信本文中のマイナンバー/クレジットカード/機密マーカー → Warn) を追加。併せて送信側 `outbound_dlp_eval` の `known_recipient_domains` が常に空でタイポドメイン誤配検出が不発だった問題を、連絡先履歴からの既知ドメイン供給で修正。**kaname-dlp 変更のため security-lead 承認要**
- fix(ci): static-check 検査9 (fuzz import 照合) が空転していた問題を修正 (D105) — heredoc stdin 実行では `__file__ = "<stdin>"` となり `abspath→dirname×2` が repo の親ディレクトリに chdir して glob が0件ヒットで常に OK を返していた。`cd` 済み cwd を使う形に修正し合成違反で検知を実測。併せて corpus ↔ target ↔ `[[bin]]` 対応を見る検査10 を追加し孤立コーパスの再発を防止
