...
### Fixed
- fix(kaname-ui): オンボーディングの「Continuity を有効化」トグルを削除 (D99) — `kaname-continuity` クレートは D19 で削除済みで、存在しない機能への opt-in を収集する placebo だった。`settings_save_onboarding` の `continuity` 引数も除去
### Fixed
