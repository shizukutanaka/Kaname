...
### Fixed
- fix(kaname-ui): オンボーディング完了画面の虚偽機能表示2件を削除 (D110) — 「⌘K コマンドパレット」(実装不存在のショートカット教示) と「Phi-4-mini AI モデルはすでに準備されています」(kaname-ai はスタブで未接続) を除去。実測で真の「BEC 検出稼働中」「DB 暗号化済み」は保持。使われなくなった `.k-tip` スタイルも削除
