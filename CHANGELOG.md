...
### Fixed
- fix(kaname-jmap): `get_email_body` が読み手ゼロの本文値を最大 ~1MB/通転送していた無駄を除去 (D93) — properties を `id`/`blobId`/`attachments` に削減し `fetchTextBodyValues`/`fetchHTMLBodyValues`/`maxBodyValueBytes`/`bodyValues` 要求を廃止。本文表示は `download_blob` (生 RFC5322) が担うため挙動変化なし。再追加を検出する回帰テスト追加
