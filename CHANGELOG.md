...
### Fixed
- fix(kaname-jmap): set 系応答の `notCreated`/`notUpdated`/`notDestroyed` を検査する `check_set_errors` を追加し `mark_read`/`trash`/`send_email` (EmailSubmission/set) に適用 — サーバーが更新や送信をアイテム単位で拒否しても transport 成功をもって「操作成功」と誤報していた (D79)
