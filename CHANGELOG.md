...
### Fixed
- fix(kaname-store,kaname-ui): ログにフルファイルパスを出力していた I5 違反を修正 (D96) — `/Users/<name>/...` の OS ユーザー名・ディレクトリ構造が漏れていた。`redact_path` で葉名のみに落とす (PrivacySanitizer がメール/トークンを覆うがパスは対象外だった残り面)
