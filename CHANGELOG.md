...
### Fixed
- fix(kaname-observability): `PrivacyLayer` が PII を検知しても元イベントを平文のまま出力していた残課題を解消 (D28) — `fmt::layer` の Writer に `SanitizingWriter` を挟み、出力バイト列を `PrivacySanitizer` に通してから書き出す実抑制層を追加。回帰テストで出力にメールアドレス/トークンが残らないことを固定
