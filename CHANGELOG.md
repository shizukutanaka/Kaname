...
### Fixed
- fix(kaname-store/kaname-ui): 既読化・ゴミ箱移動がローカル DB に反映されなかった問題を修正 (D77) — `mail_mark_read`/`mail_trash` は JMAP のみ更新し `is_read`/`is_deleted` が永遠に古いままだった (オフライン一覧・検索・未読バッジに影響)。`mark_messages_read`/`mark_message_deleted` を追加し JMAP 成功後に best-effort 同期。`save_message` upsert に `is_deleted = 0` を追加し復元が反映されるようにも修正
