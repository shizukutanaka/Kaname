...
pub async fn analyze_raw_email(bytes: &[u8]) -> Result<ImportedEmail, String> {
    ...
    if env.shiho_marks {
        render_risks.push(
            "X-ShihoShoshi-*/X-Scrivener-*/X-Registry-*/X-JudicialScrivener-* 等 — 士機の通知記録を送信側が自称する兆候です"
                .to_string(),
        );
    }
    ...
    if env.shouji_marks {
        render_risks.push(
            "X-Shouji-*/X-Fusuma-*/X-Harikae-* 等 — 襖機の通知記録を送信側が自称する兆候です"
                .to_string(),
        );
    }
    ...
    if env.kaban_marks {
        render_risks.push(
            "X-Kaban-*/X-Kawa-*/X-BagRepair-*/X-LeatherRepair-* 等 — 鞄機の通知記録を送信側が自称する兆候です"
                .to_string(),
        );
    }
