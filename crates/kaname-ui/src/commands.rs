...
pub async fn analyze_raw_email(bytes: &[u8]) -> Result<ImportedEmail, String> {
    ...
    if env.shodo_marks {
        render_risks.push(
            "X-Shodo-*/X-Shuuji-*/X-Calligraphy-* 等 — 字機の通知記録を送信側が自称する兆候です"
                .to_string(),
        );
    }
    ...
    if env.dance_marks {
        render_risks.push(
            "X-Dansu-*/X-Dance-*/X-Ballet-*/X-DanceStudio-* 等 — 舞機の通知記録を送信側が自称する兆候です"
                .to_string(),
        );
    }
    ...
    if env.nenkin_marks {
        render_risks.push(
            "X-Nenkin-*/X-Fp-*/X-FinancialPlan*-*/X-EstatePlan-*/X-Pension-* 等 — 年機の通知記録を送信側が自称する兆候です"
                .to_string(),
        );
    }
