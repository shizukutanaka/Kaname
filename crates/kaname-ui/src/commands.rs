...
/// 「送信側が書き込んだヘッダの存在」だけを根拠とする警告に共通する末尾 (D571)。
///
/// - `を送信側が自称する兆候です` — `if env.*_marks` 等のベンダー固有ヘッダ (static-check 検査12 が強制)
/// - `を自署する兆候です` — `Complaints-To`/`X-Report-Abuse`・`Feedback-ID`
/// - `を送信側が書く兆候です` — `X-Autogenerated*`/`X-Autoresponder`
///
/// DMARC 認証済みの送信者では一括で取り除く (`analyze_raw_email` 末尾の注記参照)。
const SELF_CLAIM_SUFFIXES: [&str; 3] = [
    "を送信側が自称する兆候です",
    "を自署する兆候です",
    "を送信側が書く兆候です",
];

/// 警告が「送信側が書き込んだヘッダの存在」だけを根拠とするものか。
fn is_self_claim_warning(risk: &str) -> bool {
    SELF_CLAIM_SUFFIXES.iter().any(|s| risk.ends_with(s))
}
...
pub async fn analyze_raw_email(bytes: &[u8]) -> Result<ImportedEmail, String> {
    ...
    if env.notary_marks {
        render_risks.push(
            "X-Notary-*/X-Notarize-*/X-Koushou-*/X-MobileNotary-*/X-Apostille-*/X-LoanSigning-* 等 — 証機の通知記録を送信側が自称する兆候です"
                .to_string(),
        );
    }
    ...
    if env.translation_marks {
        render_risks.push(
            "X-TransPerfect-*/X-Gengo-*/X-Honyaku-*/X-TranslationPros-*/X-InterpreterService-*/X-Tsuyaku-* 等 — 訳機の通知記録を送信側が自称する兆候です"
                .to_string(),
        );
    }

    // D700: バイク便・当日便印自称
    if env.courier_marks {
        render_risks.push(
    ...
    // D782: 外構・エクステリア工事印自称
    if env.gaikou_marks {
        render_risks.push("X-Gaikou-*/X-Ekusuteria-*/X-Exteriorworks-*/X-Exteriordesign-* 等 — 構機の通知記録を送信側が自称する兆候です".to_string());
    }
    // D783: 樹木葬・散骨印自称
    if env.jumokusou_marks {
        render_risks.push("X-Jumokusou-*/X-Sankotsu-*/X-Naturalburial-*/X-Seaburial-*/X-Treeburial-* 等 — 散機の通知記録を送信側が自称する兆候です".to_string());
    }
    // D784: 人工芝・芝張り印自称
    if env.jinkoushiba_marks {
        render_risks.push("X-Jinkoushiba-*/X-Shibahari-*/X-Artificialturf-*/X-Turfinstallation-* 等 — 芝機の通知記録を送信側が自称する兆候です".to_string());
    }
    ...
    // `Feedback-ID`・`X-Report-Abuse` 等は送信ドメイン自身のメール基盤・配信サービスが
    // 正規に付けるもので、普通のメールのほぼ全てに警告枠が出る誤警報になっていた。
    // From ドメインが DMARC pass (整合済み SPF/DKIM で送信元が保証される) なら
    // それらのヘッダは認証済み送信元の基盤が書いたものであり「自称」ではない。
    // 未認証 (fail/none・認証結果なし) のメールでは従来どおり警告する。
    // 最終変更 (style_risks) の後に置くことで、後から追加される検出器も必ず対象になる。
    if env.auth_results.dmarc == kaname_render::AuthResult::Pass {
        render_risks.retain(|r| !is_self_claim_warning(r));
    }
...
mod tests {
    ...
    /// D571 用: Gmail (`X-Gm-*`/`X-Google-*`)・Microsoft 365 (`X-Microsoft-Antispam`)・
    /// GitHub (`X-GitHub-*`)・配信サービス (`Feedback-ID`/`X-Report-Abuse`) が
    /// 正規に付けるヘッダを持つメール。
    /// `auth` に `Authentication-Results` 行 (無ければ `None`) を差し込む。
    fn vendor_headers_eml(auth: Option<&str>) -> Vec<u8> {
        let mut eml = String::from("From: alice@example.com\r\nTo: bob@example.com\r\n");
        if let Some(a) = auth {
            eml.push_str(a);
            eml.push_str("\r\n");
        }
        eml.push_str(
            "Subject: Weekly sync notes\r\n\
             ...
             X-Google-Smtp-Source: AGHT+IF\r\n\
             X-Microsoft-Antispam: BCL:0;\r\n\
             X-GitHub-Reason: mention\r\n\
             Feedback-ID: 1:campaign:list:esp\r\n\
             X-Report-Abuse: Please report abuse to abuse@esp.example\r\n\
             Content-Type: text/plain; charset=utf-8\r\n\
             \r\n\
             Notes from today's sync are attached in the doc.\r\n",
        );
        eml.into_bytes()
    }

    fn self_claim_count(r: &ImportedEmail) -> usize {
        r.body
            .render_risks
            .iter()
            .filter(|s| is_self_claim_warning(s))
            .count()
    }

    /// D571: DMARC pass の送信者では、送信基盤が正規に付けるベンダー固有ヘッダを
    /// 「送信側の自称」と警告しない (普通の Gmail/M365/GitHub メールに警告枠が出ていた)。
    #[tokio::test]
    async fn analyze_raw_email_はdmarc認証済みなら自称警告を出さない() -> Result<(), String> {
        let _serial = test_serial().await;
        reset_globals().await;
        let eml = vendor_headers_eml(Some(
        ...
        assert_eq!(
            self_claim_count(&r),
            0,
            "認証済み送信者のベンダーヘッダは自称ではない: {:?}",
            r.body.render_risks
        );
        Ok(())
    }

    /// D571: 未認証 (DMARC fail) のメールでは従来どおり自称警告を出す。
    #[tokio::test]
    async fn analyze_raw_email_はdmarc_failなら自称警告を維持する() -> Result<(), String> {
        let _serial = test_serial().await;
        reset_globals().await;
        let eml = vendor_headers_eml(Some(
        ...
        let r = analyze_raw_email(&eml).await?;
        // webmail 内部印 (X-Gm-*/X-Google-*)・EOP 印 (X-Microsoft-Antispam)・
        // SaaS 通知印 (X-GitHub-*)・Feedback-ID・abuse 報告先 (X-Report-Abuse) の 5 系統以上。
        assert!(
            self_claim_count(&r) >= 5,
            "未認証メールのベンダーヘッダは自称として警告すべき: {:?}",
            r.body.render_risks
        );
        Ok(())
    }

    /// D571: 認証結果ヘッダ自体が無い (検証不能) メールでも従来どおり警告する。
    #[tokio::test]
    async fn analyze_raw_email_は認証結果なしなら自称警告を維持する() -> Result<(), String> {
        let _serial = test_serial().await;
        reset_globals().await;
        let r = analyze_raw_email(&vendor_headers_eml(None)).await?;
        assert!(
            self_claim_count(&r) >= 5,
            "認証結果の無いメールは自称警告を維持すべき: {:?}",
            r.body.render_risks
        );
        Ok(())
    }
    ...
    async fn analyze_raw_email_はhidden_text_saltingを落として検出する() -> Result<(), String> {
        ...
        assert!(
            r.body.render_risks.iter().any(|s| s.contains("非表示")),
    ...
    async fn analyze_raw_email_は表示urlと実リンクの不一致を検出する() -> Result<(), String> {
        ...
        let r = analyze_raw_email(eml).await?;
        assert!(
            r.body.render_risks.iter().any(|s| s.contains("URL 偽装")),
