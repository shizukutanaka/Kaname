...
pub async fn analyze_raw_email(bytes: &[u8]) -> Result<ImportedEmail, String> {
    ...
    if env.barber_marks {
        render_risks.push(
            "X-Barber-*/X-Riyou-*/X-Biyou-*/X-BeautySalon-* 等 — 髪機の通知記録を送信側が自称する兆候です"
                .to_string(),
        );
    }
    ...
    if env.kimono_marks {
        render_risks.push(
            "X-Kimono-*/X-Gofuku-*/X-Kitsuke-*/X-Furisode-* 等 — 着機の通知記録を送信側が自称する兆候です"
                .to_string(),
        );
    }
    ...
    if env.jewel_marks {
        render_risks.push(
            "X-Jewel-*/X-Jewelry-*/X-Jeweller-*/X-Houseki-* 等 — 飾機の通知記録を送信側が自称する兆候です"
                .to_string(),
        );
    }
