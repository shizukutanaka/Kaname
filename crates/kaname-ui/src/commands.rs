...
pub async fn analyze_raw_email(bytes: &[u8]) -> Result<ImportedEmail, String> {
    ...
    // D824: 整水器・還元水印自称
    if env.seisuiki_marks {
        render_risks.push("X-Seisuiki-*/X-Kangensui-*/X-Kassuiki-*/X-Waterionizer-*/X-Alkalinewater-*/X-Ionizersales-* 等 — 整機の通知記録を送信側が自称する兆候です".to_string());
    }
    // D825: 水素水・活水印自称
    if env.suisosui_marks {
        render_risks.push("X-Suisosui-*/X-Suisomizu-*/X-Kassui-*/X-Hydrogenwater-*/X-H2water-*/X-Hydrogenizer-* 等 — 素機の通知記録を送信側が自称する兆候です".to_string());
    }
    // D826: 酵素・発酵健康食品印自称
    if env.kouso_marks {
        render_risks.push("X-Kouso-*/X-Hakkou-*/X-Kouzoyaku-*/X-Enzymesupp-*/X-Enzymesales-*/X-Fermentedsupp-* 等 — 酵機の通知記録を送信側が自称する兆候です".to_string());
    }
