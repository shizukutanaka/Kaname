...
pub async fn analyze_raw_email(bytes: &[u8]) -> Result<ImportedEmail, String> {
    ...
    if env.futon_marks {
        render_risks.push(
            "X-Futon-*/X-Shingu-*/X-MattressPros-*/X-Bedding-* 等 — 寝機の通知記録を送信側が自称する兆候です"
                .to_string(),
        );
    }
    ...
    if env.frame_marks {
        render_risks.push(
            "X-Gakubuchi-*/X-Gakusou-*/X-FrameShop-*/X-PictureFrame-* 等 — 額機の通知記録を送信側が自称する兆候です"
                .to_string(),
        );
    }
    ...
    if env.uranai_marks {
        render_risks.push(
            "X-Uranai-*/X-FortunePros-*/X-Psychic-*/X-Tarot-* 等 — 占機の通知記録を送信側が自称する兆候です"
                .to_string(),
        );
    }
