...
impl AiPhishingDetector {
    ...
    pub fn analyze(text: &str, subject: Option<&str>) -> AiPhishingAnalysis {
        ...
        let uniformity = if sentences.len() >= 3 {
            let lengths: Vec<f32> = sentences.iter().map(|s| s.trim().len() as f32).collect();
        ...
        let ttr_score = if !(0.35..=0.88).contains(&ttr) {
            0.6
        } else {
            0.1
        };
        ...
        let jp_matches = ai_phrases_ja
            .iter()
            .filter(|p| lower_text.contains(*p))
            .count();
        let en_matches = ai_phrases_en
            .iter()
            .filter(|p| lower_text.contains(*p))
            .count();
        ...
        let subject_score = if let Some(subj) = subject {
            ...
            let subj_matches = phishing_subject_patterns
                .iter()
...
impl AiAccessController {
    ...
    pub fn check_and_record(
        ...
        let hash_input = format!(
            "{}{}{}{}{}{:?}",
            prev_hash, email_id, operation, llm_type, timestamp, decision
        );
    ...
    pub fn verify_chain(&self) -> bool {
        ...
        for entry in &self.audit_log {
            ...
            if !ct_eq_hex(&expected, &entry.hash) {
                return false;
            }
...
impl ContactIntelligenceEngine {
    ...
    pub fn get_intelligence(&self, email_addr: &str) -> Option<ContactIntelligence> {
        ...
        // 関係強度: 双方向の通信ほど強い
        let bidirectional_bonus = if stats.sent_to > 0 && stats.received_from > 0 {
            1.5
        } else {
            1.0
        };
        ...
        let recent_30d = stats
            .interaction_unix_times
            .iter()
            .filter(|&&ts| ts >= cutoff)
            .count() as u32;
    ...
    pub fn top_contacts(&self, n: usize) -> Vec<ContactIntelligence> {
        let mut contacts: Vec<ContactIntelligence> = self
            .contacts
            .keys()
...
fn categorize_contact(email: &str, _domain: &str, received: u32, sent: u32) -> ContactCategory {
    let email_lower = email.to_lowercase();

    if email_lower.contains("noreply")
        || email_lower.contains("no-reply")
        || email_lower.contains("newsletter")
        || received > 10 && sent == 0
    {
        return ContactCategory::Newsletter;
    }
    if email_lower.contains("support")
        || email_lower.contains("helpdesk")
...
mod tests {
    ...
    fn 機密メールは警告付きで許可する() {
        ...
        assert!(matches!(
            decision,
            AiAccessDecision::AllowWithWarning { .. }
        ));
    }
    ...
    fn 公開メールは許可する() {
        let mut ctrl = AiAccessController::new();
        let decision =
            ctrl.check_and_record("pub_001", SensitivityLabel::Public, "summarize", "q-llm");
    ...
    fn data_sourcesに受信箱全体が含まれない() {
        ...
        assert!(entries[0]
            .data_sources
            .iter()
            .any(|s| s.contains("specific_email")));
    }
    ...
    fn 双方向通信で高い関係強度() {
        ...
        for i in 0..5 {
            ...
            engine.record_interaction(
                ...
                &format!("2026-04-{:02}T11:00:00Z", i + 1),
                Some(60),
                false,
            );
        }
        let intel = engine
            .get_intelligence("alice@company.com")
            .unwrap_or_else(|| panic!("test: no intel for alice@company.com"));
    ...
    fn recent_30d_zero_when_all_interactions_ancient() {
        ...
        let intel = engine
            .get_intelligence("dave@company.com")
    ...
    fn ニュースレター送信者を分類する() {
        ...
        let intel = engine
            .get_intelligence("noreply@newsletter.com")
            .unwrap_or_else(|| panic!("test: no intel for noreply@newsletter.com"));
    ...
    fn 返信要求を抽出する() {
        ...
        assert!(items
            .iter()
            .any(|i| i.action_type == ActionType::ReplyRequired));
    }
