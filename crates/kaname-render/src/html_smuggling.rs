...
pub enum SmugglingSignal {
    ...
    DataUriExecutable,
    /// クリップボードへの書き込み — ClickFix/FileFix でコマンドをコピーさせる (D1248)
    ClipboardWrite,
...
impl HtmlSmugglingDetector {
    ...
    pub fn analyze(&self, html: &str) -> SmugglingScan {
        ...
        // 8. クリップボードへの書き込み (D1248 — ClickFix/FileFix)
        ...
        // 9. 実行ダイアログ/貼り付け誘導の文言 (D1248 — ClickFix/FileFix)
...
mod tests {
    ...
    fn risk_calculation_caution_single_signal() {
        let d = detector();
        // 偽 CAPTCHA のみ — "to verify" を含むと D1248 の
    ...
    // ── D1248: ClickFix / FileFix (クリップボード書込み + 実行誘導) ─────────
